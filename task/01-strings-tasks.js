...
function getRectangleString(width, height) {
    const line = '─';
	const space = ' ';
	let result = '┌' + line.repeat(width - 2) +'┐\n';
	for (let i = 2; i < height; i++) {
        result += '│' + space.repeat(width - 2) +'│\n';
	}
    result += '└' + line.repeat(width - 2) +'┘\n';
    return result;
}
...
function encodeToRot13(str) {
    return str.replace(/[a-zA-Z]/g, arg => {
        let start = arg <= 'Z' ? 65 : 97;
        return String.fromCharCode(start + (arg.charCodeAt(0) - start + 13) % 26);
    });
}
...
function getCardId(value) {
    const marka = '♣♦♥♠';
	const numb = 'A2345678910JQK';
	let markaresult = marka.indexOf(value[value.length - 1]);
	let numbresult = numb.indexOf(value[0]);
	if (value[0] == 'J' || value[0] == 'Q' || value[0] == 'K') {
        numbresult--;
    }
	return 13 * markaresult + numbresult;
}
